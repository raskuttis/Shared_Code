...
for i in xrange(0,nfs):
    ...
    def zero_func(eps):
        return 1.0 - eps_of_all(eps, x, sigma, sigma0, psi) - eps
    eps = fminbound(min_func,1.0e-6,1.0)
    epsq = brentq(zero_func,1.0e-6,1.0)
    fitepseff.append(eps)
    fiteff.append(min_func(eps))
    fitadjeff.append(epsq)
...
plt.plot(fitsigma,fiteff,'r',fitsigma,fitepseff,'k',fitsigma,fitadjeff,'r--')
plt.xscale('log')
...
pp = PdfPages(plotdir + 'fprese.pdf')
pp.savefig()
pp.close()
