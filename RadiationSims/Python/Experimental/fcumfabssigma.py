...
for i in xrange(0,nfs):
    ...
    for j in xrange(0,ntplot):
        ...
        thist = time[tmini]
        thistdel = time[tminidel]
        tminfi = np.abs(timef - thist).argmin()
        tminfidel = np.abs(timef - thistdel).argmin()
        
        #allfabs[j,i] = prejout[tminfidel] / pradout[tminfi]
        allfabs[j,i] = (prejout[tminfi] + pturb[tmini]) / pradin[tminfi]
        ...
        print j, allfabs[j,i], allfabscum[j,i], prejout[tminfi] / pradin[tminfi], pturb[tmini] / pradin[tminfi]
...
plt.ylabel(r"$\displaystyle p_r / p_*$")
#plt.xlabel(r"$\displaystyle \Sigma_{\rm cl,0} / M_{\odot} {\rm pc^{-2}}$")
...
pp = PdfPages(plotdir + 'fcumfabssigma.pdf')
pp.savefig()
pp.close()
