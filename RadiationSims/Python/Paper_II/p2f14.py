...
for i in xrange(0,nds):
    ...
    for j in xrange(125,550):
        ...
        if (sigma > 0.0):
            ...
            x = np.sqrt(sigmacloud * msol * mtot / (mean * xdenom))
            print j, x
...
plt.axis([0,2,0.0,1.1])
plt.xticks([0,1,2])
plt.yticks([0.0,0.5,1.0])
plt.xlabel(r"$\displaystyle t / t_{\rm ff}$")
plt.ylabel(r"$\displaystyle M_{\rm gas} / M_{\rm cl, 0}$")

pp = PdfPages(plotdir + 'f14.pdf')
pp.savefig()
pp.close()
