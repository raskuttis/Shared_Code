...
## Results from Duval - Change to path where txt file is located
...
plt.plot(mheyer,sigmaheyer,'k.',mduval,sigmaduval,'r.',markersize=2.5)
...
plt.plot(vtheyer / np.sqrt(rheyer),sigmaheyer,'k.',vtduval / np.sqrt(rduval),sigmaduval,'r.',markersize=2.5)
plt.text(xcap,ycap,r"$\displaystyle(b)$")
plt.yscale('log')
plt.xscale('log')
plt.axis([1.0e-1,1.0e1,1.0,1.0e3])
plt.yticks([1.0,1.0e1,1.0e2,1.0e3])
plt.xticks([1.0e-1,1.0,1.0e1])

plt.xlabel(r"$\displaystyle v_{\rm RMS} r_0^{-1/2} / ({\rm km s^{-1}~pc^{-1/2}})$")
