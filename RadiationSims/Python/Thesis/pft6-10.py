...
for i in xrange(0,nds):
    ...
    for j in xrange(125,410):
        ...
        plotfabs.append(np.sum(plotpdf * (1.0 - np.exp(-1.0 * plottau))))
    ...
    for j in xrange(0,ntvs):
        ...
        plotsigmas[j,i] = plotfabs[tminpdfi]
        print j, tmin, plotfabs[tminpdfi]
...
plt.ylabel(r"$\displaystyle f_{\rm abs, \tau}$")

pp = PdfPages(plotdir + 'ft6-10.pdf')
pp.savefig()
pp.close()
