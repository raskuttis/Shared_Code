...
psim, plow, pmid = plt.plot(allsigma,allfabs,'k.',allsigma,np.squeeze(altvinf[:,1]) * (1.0 - alleps) / alleps, 'r+',allsigma,np.squeeze(allvinf[:,1]) * (1.0 - alleps) / alleps, 'b+')
...
plt.text(10**(3+0.05*3),0.9*60,r"$\displaystyle(a)$")
plt.xscale('log')
plt.axis([1.0e3,1.0e6,0,60.0])
plt.yticks([0,20,40,60])
plt.xticks([1.0e3,1.0e4,1.0e5,1.0e6])

plt.ylabel(r"$\displaystyle p_{\rm r} / M_{\rm *} [{\rm km~s^{-1}}]$")
...
plt.text(10**(3+0.9*3),0.9*60,r"$\displaystyle(b)$")
plt.xscale('log')
plt.axis([1.0e3,1.0e6,0,60.0])
plt.yticks([0,20,40,60])
plt.xticks([1.0e3,1.0e4,1.0e5,1.0e6])

plt.ylabel(r"$\displaystyle p_{\rm r} / M_{\rm of} [{\rm km~s^{-1}}]$")
plt.xlabel(r"$\displaystyle M_{\rm cl,0} / [M_{\odot}]$")

pp = PdfPages(plotdir + 'f23.pdf')
pp.savefig()
pp.close()
