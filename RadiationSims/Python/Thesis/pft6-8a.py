...
t1 = plt.plot(plotmfs,plotsigmas[0,:],'k.',thmfs,thsigs,'k--')
#plt.legend((t1,t2,t3), (r"$\displaystyle {\rm SFE} = 2~\%$",r"$\displaystyle {\rm SFE} = 10~\%$",r"$\displaystyle {\rm SFE} = 30~\%$"),prop={'size':8},loc=1)
plt.text(10**(-0.3+0.05*2.3),0.9*1.5,r"$\displaystyle(a)$")
plt.xscale('log')
plt.axis([0.5,1.0e2,0,1.5])
...
#plt.subplot(1,2,2)
#t1 = plt.plot(plotmfvars[0,:],plotsigmas[0,:],'k.',thmfs,thsigbs,'k--')
#plt.text(10**(0+0.05*2),0.9*1.5,r"$\displaystyle(b)$")
#plt.xscale('log')
#plt.axis([1.0,1.0e2,0.,1.5])
#plt.yticks([0,0.5,1,1.5],[' ',' ',' ',' '])
#plt.xticks([1.0,1.0e1,1.0e2])
...
pp = PdfPages(plotdir + 'ft6-8a.pdf')
pp.savefig()
pp.close()
