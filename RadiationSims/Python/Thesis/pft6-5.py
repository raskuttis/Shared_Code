...
for i in xrange(0,nds):
    ...
    for j in xrange(0,nps):
        ...
        tmini = np.abs(mstar - 0.01).argmin()
        tmin = time[tmini]
...
plt.legend((pa, pb, pc), (r"$\displaystyle {\rm Run~I}$",r"$\displaystyle {\rm Run~MII}$",r"$\displaystyle {\rm Run~MI}$"),prop={'size':8},loc=1)
plt.xscale('log')
plt.yscale('log')
