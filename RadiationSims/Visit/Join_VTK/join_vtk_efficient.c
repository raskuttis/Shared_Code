 ...
 *   file for visualization and analysis.  The code is not optimal and is a bit
 *   slower than its predecessor since it opens/closes each file so that only
 *   one file is open at a time, which uses less memory.
 *
...
/* Changed the file access so that only one file is open at a time, 
   which is slower, but uses less memory.  
   -- Aaron Skinner -- Nov. 30, 2013 */
...
typedef struct Domain_s{
  char *fname;
  char *comment;
  long cur_pos;      /* Offset (in bytes) to the current position */
...
static void init_domain_1d(void){
  ...
  float time;
  int cycle;
  int n, npoints, tmp1,tmp2,*tmp3;

  for(i=0; i<file_count; i++){
    if((fp = fopen(domain_1d[i].fname,"r")) == NULL)
    ...
    if(strcmp(line,"DATASET STRUCTURED_POINTS") != 0)
      join_error("Unsupported file type: %s\n",line);
    ...
    /* Time, Cycle */
    fscanf(fp,"FIELD FieldData 2\n");
    fscanf(fp,"TIME 1 1 float\n");
    fread(&time, sizeof(float), 1, fp);
    fscanf(fp,"\n");
    domain_1d[i].time = time;
    ath_bswap(&time,sizeof(float),1);
    printf("FIELD FieldData 2\n");
    printf("TIME 1 1 float\n");
    printf("%le\n",time);
    fscanf(fp,"CYCLE 1 1 int\n");
    fread(&cycle, sizeof(int), 1, fp);
    fscanf(fp,"\n");
    domain_1d[i].cycle = cycle;
    ath_bswap(&cycle,sizeof(int),1);
    printf("CYCLE 1 1 int\n");
    printf("%d\n",cycle);
    ...
    if(cell_dat != ndat)
      join_error("Error:  Nx*Ny*Nz = %d\n",ndat);
    
    /* Save current position, close file */
    domain_1d[i].cur_pos = ftell(fp);
    fclose(fp);
  }
...
//
//        /* Open file, jump to the current position */
//        if((fp_in = fopen(domain_3d[kg][jg][ig].fname,"r")) == NULL)
//          join_error("Error opening file \"%s\"\n",domain_3d[kg][jg][ig].fname);
//        fseek(fp_in,domain_3d[kg][jg][ig].cur_pos,SEEK_SET);
//
//	      if((nread = fread(&fdat, sizeof(float), 1, fp_in)) != 1)
//          join_error("read_write_scalar error\n");
//
//	      fwrite(&fdat, sizeof(float), 1, fp_out);
//        
//        /* Save current position, close file */
//        domain_3d[kg][jg][ig].cur_pos = ftell(fp_in);
//        fclose(fp_in);
//	    }
//	  }
//	}
	...
	for(j=0; j<domain_3d[0][jg][0].Ny; j++){
	  for(ig=0; ig<NGrid_x; ig++){
        ...
        fseek(fp_in,domain_3d[kg][jg][ig].cur_pos,SEEK_SET);
	      ...
	      if((nread = fread(fvec, sizeof(float), 3, fp_in)) != 3)
...
static void write_joined_vtk(const char *out_name){
  ...
  nxt = nyt = nzt = 0;
  for(ig=0; ig<NGrid_x; ig++)
    nxt += domain_3d[0][0][ig].Nx;
  
  for(jg=0; jg<NGrid_y; jg++)
    nyt += domain_3d[0][jg][0].Ny;
  
  for(kg=0; kg<NGrid_z; kg++)
    nzt += domain_3d[kg][0][0].Nz;
  ...
  fprintf(fp_out,"FIELD FieldData 2\n");
  fprintf(fp_out,"TIME 1 1 float\n");
  fwrite(&(domain_3d[0][0][0].time), sizeof(float), 1, fp_out);
  fprintf(fp_out,"\n");
  fprintf(fp_out,"CYCLE 1 1 int\n");
  fwrite(&(domain_3d[0][0][0].cycle), sizeof(int), 1, fp_out);
  fprintf(fp_out,"\n");
    ...
    for(kg=0; kg<NGrid_z; kg++){
      for(jg=0; jg<NGrid_y; jg++){
        for(ig=0; ig<NGrid_x; ig++){
          ...
          assert(fseek(fp_in, -1, SEEK_CUR) == 0);
          while (isspace(fgetc(fp_in)))
            assert(fseek(fp_in, -2, SEEK_CUR) == 0);
          assert(fgetc(fp_in) == '\n');
