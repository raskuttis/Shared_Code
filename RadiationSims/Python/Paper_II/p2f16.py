...
plt.ylabel(r"$\displaystyle {\rm Absorption~Fraction}$")
plt.text(0.05*2,0.9*1,r"$\displaystyle(a)$")

plt.subplot(1,2,2)
ft, ftnf, ff = plt.plot(timef,fabstauout,'k',timenf,fabstaunfout,'--k',timef,fabsfout,'r')
plt.legend((ft, ff), (r"$\displaystyle f_{\rm abs, \tau}$",r"$\displaystyle f_{\rm abs, F}$"),prop={'size':8},loc=3)
plt.axis([0,2,0,1.0])
plt.xticks([0,1,2])
plt.yticks([0,0.5,1],[' ',' ',' '])
