function FV = demo_fv(GENDATA, numClusters, exemplarsize, pcaval)
...
'Starting pca'
if pcaval <= 99.9
    [coeff,score,latent,tsquared,explained,mu] = pca(transpose(allexedata));
    'Finished pca'

    % Select enough components to reach pcaval
    cs = cumsum(explained);
    ncoeffs = min(find(cs > pcaval));
    goodcoeff = coeff(:,1:ncoeffs);
    allexedata = transpose((transpose(allexedata) - repmat(mu,nex,1)) * goodcoeff);

    for j = 1 : length(GENDATA.data)
        sizeexe = size(EXE{j});
        nex = sizeexe(2);
        EXE{j} = transpose((transpose(EXE{j}) - repmat(mu,nex,1)) * goodcoeff);
    end
end
'Finished pca loop'
