...
for i in xrange(0,nds):
    ...
    for j in xrange(125,390):
        ...
        if startflag:
            amp, mu, sigma, chisq = sim_fitlnpdfvals(plotpdf,plotlogden,plotpdf,0.25,0.95,nvals,pin=[ampold,muold,sigmaold])
            ...
            amp, mu, sigma, chisq = sim_fitlnpdfvals(plotpdf,plotlogden,plotpdf,0.25,0.95,nvals)
    ...
    for j in xrange(0,ntvs):
        ...
        print j, tmin, plotsigma[tminpdfi]
...
plt.subplot(1,2,2)
t1 = plt.plot(plotmfs,ploteffs,'k.')
plt.text(10**(0+0.05*2),0.9*1+0.5,r"$\displaystyle(b)$")
plt.xscale('log')
plt.axis([1.0,1.0e2,0.0,1.0])
plt.yticks([0.0,0.5,1])
plt.xticks([1.0,1.0e1,1.0e2])

plt.xlabel(r"$\displaystyle \mu_{\Phi}$")
#plt.ylabel(r"$\displaystyle \sigma_{{\rm ln}\Sigma}$")

pp = PdfPages(plotdir + 'ft6-13.pdf')
pp.savefig()
pp.close()
