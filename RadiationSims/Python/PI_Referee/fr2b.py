...
for i in xrange(0,nds):
    ...
    if (i == 0):
        ...
        tmin = time[tmini]
        tminhi = np.abs(mstar - 0.9 * eff).argmin()
        tminh = time[tminhi]
    ...
    plotfitm = sim_fitlnpdf(plotpdfm,plotlogden,plotpdfm,0.1,0.9,nvals)
    ...
    tminpdfi = np.abs(pdftime - tminh).argmin()
...
#plt.plot(plotdens[0],plotfitms[0],'r--',plotdens[1],plotfitms[1],'k--')
plt.xscale('log')
plt.yscale('log')
...
#plt.plot(plotdens[0],altplotfitms[0],'r--',plotdens[1],altplotfitms[1],'k--')
plt.xscale('log')
plt.yscale('log')
