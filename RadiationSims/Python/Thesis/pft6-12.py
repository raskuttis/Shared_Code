...
dflist = ['UV_M5.0e4_R15.0_N256_Tf4_Fluxes/', 'UV_M5.0e4_R15.0_N256_Tf4_B5.0_LD_Fluxes/', 'UV_M5.0e4_R15.0_N256_Tf4_B0.05_LD_Fluxes/']
nfs = len(dflist)
nts = 100
...
for i in xrange(0,nfs):
    ...
    for j in xrange(0,nts):
        ...
        rdist = star_sigma(stardata)
        tmini = np.abs(time - plotts[j]).argmin()
        allfs[i,j] = rdist[tmini] / rcloud
...
plt.legend((rc, dc, mc), (r"$\displaystyle {\rm Run~I}$",r"$\displaystyle {\rm Run~MIII}$",r"$\displaystyle {\rm Run~MI}$"),prop={'size':8},loc=1)
plt.axis([0,3,0,2])
plt.xticks([0,1,2,3])
plt.yticks([0,1,2])
...
pp = PdfPages(plotdir + 'ft6-12.pdf')
pp.savefig()
pp.close()
