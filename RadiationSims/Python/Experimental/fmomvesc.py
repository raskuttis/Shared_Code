...
## surface density and radius
    ...
    print i, allsigma[i], allfabs[i], altfabs[i], allvinf[i,:], altvinf[i,:], voutsq
...
plt.text(0.05*12,0.9*100,r"$\displaystyle(a)$")
#plt.xscale('log')
plt.axis([0.0,12.0,0,100.0])
plt.yticks([0,25,50,75,100])
plt.xticks([0.0,4.0,8.0,12.0])

plt.ylabel(r"$\displaystyle p_{\rm r} / M_{\rm *} ({\rm km/s})$")
plt.xlabel(r"$\displaystyle v_{\rm esc} ({\rm km/s})$")
...
plt.text(0.05*12,0.9*60,r"$\displaystyle(b)$")
#plt.xscale('log')
plt.axis([0.0,12.0,0,60.0])
...
pp = PdfPages(plotdir + 'fmomvesc.pdf')
pp.savefig()
pp.close()
